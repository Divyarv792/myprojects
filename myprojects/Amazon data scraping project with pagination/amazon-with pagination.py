...
def datas(soup):
    ...
    with open(r'C:\Users\Naveen\Desktop\data science\data scrapping\Amazon data scraping project with pagination\data.csv','w', newline='',encoding='UTF8')as f:
        writer=csv.writer(f)
    ...
    for product in products: 
        ...
        if product.find('span',class_='a-icon-alt'):
            product_rating=product.find('span',class_='a-icon-alt').text.split(' ')
        else:
    ...
    #header=['Product Name','Price','Rating','No of Reviews','ASIN','Manufacturer','URL']
        ...
        with open(r'C:\Users\Naveen\Desktop\data science\data scrapping\Amazon data scraping project with pagination\data.csv','a+', newline='',encoding='UTF8')as f:
            writer=csv.writer(f)
...
def next_page(soup):
    ...
    if not page.find('span',{'class':'s-pagination-item s-pagination-next s-pagination-disabled '}):
        next_url='https://www.amazon.in' + str(soup.find('a',{'class':'s-pagination-item s-pagination-next s-pagination-button s-pagination-separator'})['href'])
        #ss=get_datas(next_url)
        #dd=datas(ss)
        return next_url
    else:
        return
